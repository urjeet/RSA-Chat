 ...
 *     	RSAKeys.txt where the RSA keys are stored
 *     	SymmetricCipher.java, the interface for the symmetric ciphers
 *     	Substitution.java, the substitution cipher
 *     	Additive.java, the addition cipher
 *	Adapted from Dr. John Ramirez's CS1501
 */
public class RSAChatServer {
	...
	public RSAChatServer(int MaxU) throws IOException
	{
		MaxUsers = MaxU;
		...
		Scanner inScan = new Scanner(new File("RSAKeys.txt"));
	...
	public synchronized void SendMsg(String msg)
	{
		for (int i = 0; i < numUsers; i++)
		{
			...
			SymmetricCipher currCipher = threads[i].getCipher();
			...
			while (true)
			{
				if (numUsers < MaxUsers)
				{
					try
					{
						...
						{
							System.out.println("Additive Cipher Chosen");
							cipher = new Additive(byteKey);
						}
						else
						{
							System.out.println("Substitution Cipher Chosen");
							cipher = new Substitution(byteKey);
						}
						...
						// Get the Client's name (note that this is encoded).
	...
	// client is a new UserThread object, with the data shown.  The cipher
	// being used is encapsulated within the UserThread, so that each client can have
	// a different encryption scheme, as long as it satisfies the SymmetricCipher interface.
	...
	private class UserThread extends Thread
	{
		...
		private SymmetricCipher myCipher;
		private int myId;
		private String myName;

		private UserThread(Socket newSocket, int id, String newName,
				ObjectInputStream newReader,
				ObjectOutputStream newWriter, SymmetricCipher c) throws IOException
		{
			mySocket = newSocket;
			myId = id;
			myName = newName;
			...
			while (ok)
			{
				...
				try {
					...
					if (newBytes == null || newMsg.equals("CLIENT CLOSING"))
						...
						RSAChatServer.this.SendMsg(newMsg);
				}
				catch (Exception e)
				{
	...
	{
		RSAChatServer Secure = new RSAChatServer(30);
	}
}
